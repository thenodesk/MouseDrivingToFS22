...
function RoyalMod.new(debug, mpSync)
    ...
    mod.gameEnv = modEnvMeta.__index
    mod.super = {}
    mod.debug = debug
    ...
    mod.super.errorHandle = function(error)
        ...
        print("DevError: RoyalMod caught error from " .. mod.name .. " (" .. mod.version .. ")")
    ...
    if mpSync then
        ...
        mod.super.sync.writeStream = function(self, streamId)
            ...
            if mod.onWriteStream ~= nil then
                ...
                print("[" .. mod.name .. "] Written " .. offset .. " bits (" .. (offset / 8) .. " bytes) in " .. netGetTime() - time .. " ms")
            end
        end
        ...
        mod.super.sync.readStream = function(self, streamId)
            ...
            if mod.onReadStream ~= nil then
                ...
                print("[" .. mod.name .. "] Read " .. offset .. " bits (" .. (offset / 8) .. " bytes) in " .. netGetTime() - time .. " ms")
            end
        end
    ...
    mod.super.oldFunctions.VehicleTypeManagervalidateVehicleTypes = TypeManager.validateTypes
    ...
    mod.super.oldFunctions.VehicleTypeManagerfinalizeVehicleTypes = TypeManager.finalizeTypes
        ...
        if self.typeName == "vehicle" then
                ...
                xpcall(
                    ...
                    function(specName)
                        if not specAddAllowed then
                            ...
                            print("DevError: [" .. mod.name .. "] addSpecialization is no more allowed")
                            return
                        end
                    ...
                    function(specName, requiredSpecName)
                        if not specAddAllowed then
                            ...
                            print("DevError: [" .. mod.name .. "] addSpecializationBySpecialization is no more allowed")
                            return
                        end
                    ...
                    function(specName, requiredVehicleTypeName)
                        if not specAddAllowed then
                            ...
                            print("DevError: [" .. mod.name .. "] addSpecializationByVehicleType is no more allowed")
                            return
                        end
                    ...
                    function(specName, func)
                        if not specAddAllowed then
                            ...
                            print("DevError: [" .. mod.name .. "] addSpecializationByFunction is no more allowed")
                            return
                        end
            ...
            for i, spec in pairs(specs) do
                if g_specializationManager:getSpecializationByName(spec.name) == nil then
                    ...
                    print("DevError: [" .. mod.name .. "] Can't find specialization " .. spec.name)
                ...
                for _, spec in pairs(specs) do
                    if typeEntry.specializationsByName[spec.name] == nil then
                        if g_specializationManager:getSpecializationObjectByName(spec.name).prerequisitesPresent(typeEntry.specializations) then
                            ...
                            print("DevError: [" .. mod.name .. "] Not all prerequisites of specialization " .. spec.name .. " are fulfilled by " .. typeName)
                        end
                    end
                end
            ...
            for _, spec in pairs(specs) do
                if #spec.addedTo <= 25 then
                    ...
                    print("DevInfo: [" .. mod.name .. "] " .. spec.name .. " added to [" .. table.concat(spec.addedTo, ", ") .. "]")
                else
                    --g_logManager:devInfo("[%s] %s added to %s vehicle types out of %s", mod.name, spec.name, #spec.addedTo, vehicleTypesCount)
                    print("DevInfo: [" .. mod.name .. "] " .. spec.name .. " added to " .. #spec.addedTo .. " vehicle types out of " .. vehicleTypesCount)
                end
            end
    ...
    BaseMission.loadMapFinished = function(self, mapNode, arguments, callAsyncCallback, ...)
        --print(getArgs(mod.super.oldFunctions.BaseMissionloadMapFinished))
        mod.mapNode = mapNode
        --local mapFile, _, _, _ = unpack(arguments)
        local mapFile = arguments
