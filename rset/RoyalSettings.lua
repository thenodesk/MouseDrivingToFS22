...
function RoyalSettings:initialize()
    ...
    print("DevInfo: Initializing Royal Settings from " .. self.loadingModName)
end
...
function RoyalSettings:registerMod(modName, icon, description)
    if self.registrationEnabled then
        if self.mods[modName] == nil then
            ...
            print("DevError: [Royal Settings] Tab for '" .. modName .. "' is already registered")
        end
    else
        --g_logManager:devError("[Royal Settings] Mods / tabs registration isn't allowd at this time")
        print("DevError: [Royal Settings] Mods / tabs registration isn't allowd at this time")
    end
end
        ...
        if self.mods[modName] ~= nil then
            ...
            if sclass ~= nil then
                ...
                if self.settings[key] == nil then
                    ...
                    print("DevError: [Royal Settings] Setting with key '" .. key .. "' is already registered")
                end
            else
                ...
                print("DevError: [Royal Settings] Can't find a proper setting class for (" .. settingType .. ", " .. settingOwner .. ") type / owner combination")
            end
        else
            --g_logManager:devError("[Royal Settings] Tab for '%s' isn't still registered", modName)
            print("DevError: [Royal Settings] Tab for '" .. modName .. "' isn't still registered")
        end
    else
        --g_logManager:devError("[Royal Settings] Settings registration isn't allowd at this time")
        print("DevError: [Royal Settings] Settings registration isn't allowd at this time")
    end
end
...
function RoyalSettings.createGui(xmlFile, name, controller, isFrame)
    ...
    if xmlFile ~= nil and xmlFile ~= 0 then
        ...
        print("Error: Could not use gui-config '" .. xmlFile .. "'!")
    end
...
if game.g_royalSettings == nil then
    ...
    game.g_royalSettings.bootstrap = function()
        if game.g_royalSettings.instances ~= nil then
            local hRevision = -1
            ---@type RoyalSettings
            local object = nil
    ...
    Utility.prependedFunction(TypeManager, "validateTypes", game.g_royalSettings.bootstrap)
end
