...
function RoyalHud:getNormalizedPosition(x, y)
    ...
    local values = GuiUtils.getNormalizedValues({x, y}, {g_referenceScreenWidth, g_referenceScreenHeight})
...
function RoyalHud:getNormalizedSize(width, height)
    ...
    local values = GuiUtils.getNormalizedValues({width, height}, {g_referenceScreenWidth, g_referenceScreenHeight})
    return {values[1] * g_aspectScaleX, values[2] * g_aspectScaleY}
end
